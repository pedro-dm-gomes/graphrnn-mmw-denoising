...
class MMW(object):
    def __init__(
        ...
        if not(train):
            ...
            path_to_files = root + '/' +str(num_points)+ '/train'
            npy_files = os.listdir(path_to_files)
            # For each run calculate the limit
            for run in npy_files:
            	file_path = os.path.join(path_to_files, run)
            ...
            path_to_files = root + '/' +str(num_points)+ '/test'            
            npy_files = os.listdir(path_to_files)
            for run in npy_files:
            	file_path = os.path.join(path_to_files, run)
            	npy_run = np.load(file_path)
