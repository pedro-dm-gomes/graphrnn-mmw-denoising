...
Modules Index:
	...
	Simplified version of Graph-RNN does message passing convoltuion
	
	3.Simple_GraphRNNCell_bn:
	-- Equal to Simple_GraphRNNCell, uses tf_util, has batch normalization
	
	2. Displacement_GraphRNNCell_bn :
	-- does not take point cordinates into account --
...
"""
================================================== 
              Simple_GraphRNNCell_bn                       
==================================================
"""
class Simple_GraphRNNCell_bn(object):
    def __init__(self,
                 radius,
                 nsample,
                 out_channels,
...
"""
================================================== 
              Displacement_GraphRNNCell_bn                       
==================================================
"""
class Displacement_GraphRNNCell_bn(object):
    def __init__(self,
                 radius,
                 nsample,
                 out_channels,
    ...
    def __call__(self, inputs, states):
        ...
        if X1 is not None:
        	...
        	concatenation = tf.concat([X1_expanded_to_P1, S1_grouped], axis=3)         
        	concatenation = tf.concat([concatenation, displacement, displacement_time], axis=3)
        	concatenation_2 = tf.concat([X1_expanded_to_P2, S2_grouped], axis=3)
        	concatenation_2 = tf.concat([concatenation_2, displacement_2, displacement_time_2], axis=3)    
        else:
        	""" Concatenation (t-1) = [P1_expanded, displacement_Pij | displacement_Fij| displacement_T] """
        	P1_expanded_to_P1 = tf.tile(tf.expand_dims(P1, 2), [1, 1, nsample_P1, 1])   
        	...
        	concatenation = tf.concat([displacement, displacement_time], axis=3)
        	concatenation_2 = tf.concat([displacement_2,displacement_time_2], axis=3)        
        ...
        with tf.variable_scope('graph-rnn') as sc:
        	#S1 = tf.layers.conv2d(inputs=concatenation, filters=out_channels, kernel_size=1, strides=1, padding='valid', data_format='channels_last', activation=activation,  name='fc')
        	S1 = tf_util.conv2d(inputs=concatenation, num_output_channels=out_channels,kernel_size = [1,1],  stride=[1,1], padding='VALID', is_training=is_training, activation_fn=None, bn = True, bn_decay = bn_decay, scope = 'fc')
