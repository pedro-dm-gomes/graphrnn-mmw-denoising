...
def pointnet_fp_module_original_interpolated(xyz1,
    ...
    with tf.variable_scope(scope) as sc:
        ...
        if (BN_FLAG) : new_points1 = tf.layers.batch_normalization(new_points1, training=is_training, name='bn')
...
def pointnet_sa_module(xyz, points, npoint, radius, nsample, mlp, mlp2, group_all, is_training, bn_decay, scope, bn=True, pooling='max', knn=True, use_xyz=True, use_nchw=False):
    ...
    with tf.variable_scope(scope) as sc:
        ...
        if mlp2 is not None:
            if c: new_points = tf.transpose(new_points, [0,3,1,2])
