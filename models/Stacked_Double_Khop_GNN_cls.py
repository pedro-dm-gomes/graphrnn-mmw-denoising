...
def placeholder_inputs(batch_size, seq_length, num_points):
  ...
  rotated_pointclouds_pl = tf.placeholder(tf.float32, shape=(batch_size,seq_length, num_points, 3))
  labels_pl = tf.placeholder(tf.int32, shape=(batch_size, seq_length, num_points,1 ))
  
  return pointclouds_pl, rotated_pointclouds_pl,  labels_pl
  
def get_model(point_cloud, rotated_point_cloud, is_training, model_params):
  ...
  # Without Rotated Point Cloud
  rotated_point_cloud =  point_cloud
  print("rotated_point_cloud", rotated_point_cloud)
  ...
  rotated_point_cloud = tf.reshape(rotated_point_cloud, (batch_size, seq_length * original_num_points, 3) )
  timestep_tensor = tf.reshape(timestep_tensor, (batch_size,seq_length *original_num_points, 1) )
  
  """ -----  Layer 0    -----  """
  l0_xyz = rotated_point_cloud
  l0_feats = timestep_tensor
  l0_feats = tf.concat( (point_cloud,l0_feats) ,axis = 2)
  ...
  # Create adjacent matrix on new cordinate spa space
