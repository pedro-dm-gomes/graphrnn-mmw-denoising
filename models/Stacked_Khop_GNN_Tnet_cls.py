...
def get_model(point_cloud, is_training, model_params):
  ...
  # Learn new Canonnical space
  l0_feats_expanded = tf.expand_dims(l0_feats, 2)
  print("l0_feats_expanded", l0_feats_expanded) 
  #Learn a T-NET for new space
  with tf.variable_scope('transform_net0') as sc:
      transform = feature_transform_net(l0_feats_expanded, is_training, bn_decay, K=3)
  ...
  l0_adj_matrix = tf_util.pairwise_distance(l0_xyz_transformed)
  ...
  # Learn new Canonnical space
  l1_feats_expanded = tf.expand_dims(l1_feats, 2)
  #Learn a T-NET for new space
  with tf.variable_scope('transform_net1') as sc:
      transform = feature_transform_net(l1_feats_expanded, is_training, bn_decay, K=3)
  print("transform", transform)
  l1_xyz_transformed = tf.matmul(l1_xyz, transform)
  
  
  # Create adjacent matrix on new cordinate spa ce
  l1_adj_matrix = tf_util.pairwise_distance(l1_xyz_transformed)
  ...
  # Learn new Canonnical space
  l2_feats_expanded = tf.expand_dims(l2_feats, 2)
  #Learn a T-NET for new space
  with tf.variable_scope('transform_net2') as sc:
      transform = feature_transform_net(l2_feats_expanded, is_training, bn_decay, K=3)
  print("transform", transform)
  l2_xyz_transformed = tf.matmul(l2_xyz, transform)
  

  # Create adjacent matrix on cordinate space
  l2_adj_matrix = tf_util.pairwise_distance(l2_xyz_transformed)
  ...
  # Learn new Canonnical space
  l3_feats_expanded = tf.expand_dims(l3_feats, 2)
  #Learn a T-NET for new space
  with tf.variable_scope('transform_net3') as sc:
      transform = feature_transform_net(l3_feats_expanded, is_training, bn_decay, K=3)
  print("transform", transform)
  l3_xyz_transformed = tf.matmul(l3_xyz, transform)
  
  
  # Create adjacent matrix on cordinate space
  l3_adj_matrix = tf_util.pairwise_distance(l3_xyz_transformed)
