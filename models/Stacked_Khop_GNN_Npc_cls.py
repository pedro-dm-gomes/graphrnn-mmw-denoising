...
def get_model(point_cloud, is_training, model_params):
  ...
  # Normalize the point_cloud
  norm_pc = tf.zeros(l0_xyz.shape) + l0_xyz
  min_coords = tf.reduce_min(norm_pc, axis=0)
  max_coords = tf.reduce_max(norm_pc, axis=0)
  norm_pc  = norm_pc- min_coords
  ranges = max_coords - min_coords
  norm_pc = norm_pc / ranges

  print("norm_pc", norm_pc)
  
  
  # Create adjacent matrix on cordinate space
  l0_adj_matrix = tf_util.pairwise_distance(norm_pc)
  ...
  l1_adj_matrix = tf_util.pairwise_distance(norm_pc)
  ...
  # Create adjacent matrix on cordinate space
  l2_adj_matrix = tf_util.pairwise_distance(norm_pc)
  ...
  # Create adjacent matrix on cordinate space
  l3_adj_matrix = tf_util.pairwise_distance(norm_pc)
