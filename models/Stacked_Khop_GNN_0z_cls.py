...
def get_model(point_cloud, is_training, model_params):
  ...
  stacked_num_points = original_num_points * seq_length
  ...
  xy = l0_xyz[:,:,0:2]
  zeros = tf.zeros((l0_xyz.shape[0], l0_xyz.shape[1], 1)) 
  xy_space_l0 = tf.concat( (xy,zeros), axis = 2)
  print("xy_space_l0", xy_space_l0)
  
  
  # Create adjacent matrix on cordinate space
  l0_adj_matrix = tf_util.pairwise_distance(xy_space_l0)
  ...
  # Explore the X-Y structure
  xy = l1_xyz[:,:,0:2]
  zeros = tf.zeros((l1_xyz.shape[0], l1_xyz.shape[1], 1)) 
  xy_space_l1 = tf.concat( (xy,zeros), axis = 2)
  


  # Create adjacent matrix on new cordinate spa ce
  l1_adj_matrix = tf_util.pairwise_distance(xy_space_l1)
  ...
  # Explore the X-Y structure
  xy = l2_xyz[:,:,0:2]
  zeros = tf.zeros((l2_xyz.shape[0], l2_xyz.shape[1], 1)) 
  xy_space_l2 = tf.concat( (xy,zeros), axis = 2)
  

  # Create adjacent matrix on cordinate space
  l2_adj_matrix = tf_util.pairwise_distance(xy_space_l2)
  ...
  # Explore the X-Y structure
  xy = l3_xyz[:,:,0:2]
  zeros = tf.zeros((l3_xyz.shape[0], l3_xyz.shape[1], 1)) 
  xy_space_l3 = tf.concat( (xy,zeros), axis = 2)
  
  # Create adjacent matrix on cordinate space
  l3_adj_matrix = tf_util.pairwise_distance(xy_space_l3)
