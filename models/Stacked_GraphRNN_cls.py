...
def get_model(point_cloud, is_training, model_params):
  ...
  graph_cell1 = graph_module(radius= 0.1, nsample=num_samples, out_channels=out_channels, knn= True, pooling='max')
  graph_cell2 = graph_module(radius= 0.1, nsample=num_samples, out_channels=out_channels, knn= True, pooling='max')
  graph_cell3 = graph_module(radius= 0.1, nsample=num_samples, out_channels=out_channels, knn= True, pooling='max')
  ...
  stacked_frames = 4 # Counting with previous fraems
  ...
  for i in range(int(seq_length) ):  #Interate for all the Frames
  	...
  	""" ===  Stacking of frames === """
  	print("Frame: ", i)
  	start = np.maximum(i - stacked_frames, 0) # Return 0
  	end  = np.maximum(i - 1, 0) # Return 0
  	print("Load: start: ", start)
  	print("Load: end: ", end)
  	stacked_xyz3 = tf.reshape(list_xyx3[start:end] , (batch_size, ( end - start ) * int(sampled_points_down3), 3) )
  	stacked_states3 = tf.reshape(list_states3[start:end] , (batch_size, ( end - start ) * int(sampled_points_down3), out_channels) )
  	stacked_time3 = tf.reshape(list_time3[start:end] , (batch_size, ( end - start ) * int(sampled_points_down3), 1) )
  	print("stacked_xyz3", stacked_xyz3)  	
  	print("stacked_time3", stacked_time3)  	
  	...
  	time1= tf.fill( (xyz1.shape[0],xyz1.shape[1],1), (i/1.0))

  	""" Graph-rnn cell 1 """
  	# Stack Frames
  	if(global_state1 is not None): global_state1 = tf.concat( (stacked_xyz3,s_xyz1), axis =1), s_color1, s_feat1, tf.concat( (stacked_states3,s_states1), axis =1), tf.concat( (stacked_time3,s_time1), axis =1)    
  	with tf.variable_scope('graphrnn_1', reuse=tf.AUTO_REUSE) as scope:
  		global_state1 = graph_cell1( (xyz1, None, feat1, None, time1, bn_decay, is_training), global_state1)
  		s_xyz1, s_color1, s_feat1, s_states1, s_time1  = global_state1
  	...
  	time2 = tf.fill( (xyz2.shape[0],xyz2.shape[1],1), (i/1.0))
	
  	""" Graph-rnn cell 2 """
  	# Stack Frames
  	if(global_state2 is not None): global_state2 = tf.concat( (stacked_xyz3,s_xyz2), axis =1), s_color2, s_feat2, tf.concat( (stacked_states3,s_states2), axis =1), tf.concat( (stacked_time3,s_time2), axis =1)    
  	with tf.variable_scope('graphrnn_2', reuse=tf.AUTO_REUSE) as scope:
  		global_state2 = graph_cell2( (xyz2, None, feat2, states2, time2, bn_decay, is_training), global_state2)
  		s_xyz2, s_color2, s_feat2, s_states2, s_time2 = global_state2
  	...
  	time3 = tf.fill( (xyz3.shape[0],xyz3.shape[1],1), (i/1.0))
	
  	""" Graph-rnn cell 3 """
  	# Stack Frames
  	if(global_state3 is not None): global_state3 = tf.concat( (stacked_xyz3,s_xyz3), axis =1), s_color3, s_feat3, tf.concat( (stacked_states3,s_states3), axis =1), tf.concat( (stacked_time3,s_time3), axis =1)    
  	with tf.variable_scope('graphrnn_3', reuse=tf.AUTO_REUSE) as scope:
  		global_state3 = graph_cell3( (xyz3, None, feat3, states3, time3, bn_decay, is_training), global_state3)
  		s_xyz3, s_color3,s_feat3, s_states3, s_time3 = global_state3

  	""" Stacking of frames """  
  	list_xyx3.append(s_xyz3)
  	list_states3.append(s_states3)
  	...
  	with tf.variable_scope('fc', reuse=tf.AUTO_REUSE) as scope:
  		# FC 1 Layer
  		predicted_label = tf.layers.conv1d(inputs=l0_feat, filters=out_channels, kernel_size=1, strides=1, padding='valid', data_format='channels_last', activation=tf.nn.relu, name='fc1')
  		...
  		#predicted_label = drop_layer(predicted_label, training=True)
  ...
  end_points = {'list_xyx3': list_xyx3}
